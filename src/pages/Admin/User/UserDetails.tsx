...
import { Mail, ShieldCheck, CircleUserRound, BadgeCheck, Activity, IdCard, ArrowLeft, Pencil, Phone } from "lucide-react";
import { SkeletonUserDetails } from "@/components/modules/Admin/User/SkeletonUserDetails";
      ...
      <motion.div initial="hidden" animate="visible" className="mx-auto max-w-6xl">
        <motion.div variants={fadeUp} custom={0} className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-linear-to-r from-purple-500 to-blue-500 shadow-lg transition-all duration-300 hover:shadow-purple-500/30 py-2.5 px-4 text-sm font-semibold text-white"
          >
        ...
        >
          ...
          <div className="relative h-48 overflow-hidden bg-linear-to-r from-purple-500 to-blue-500 md:h-60">
            ...
            >
              <div>
                <p className="text-sm font-medium uppercase tracking-[0.22em] text-white/70">User Profile</p>
                <h1 className="mt-2 text-2xl font-bold text-white md:text-4xl">User Details</h1>
              ...
              <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                  <motion.div whileHover={{ scale: 1.03 }} className="shrink-0">
                    <img
                      src={
                        picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(name || "User")}&background=0f172a&color=ffffff&size=256`
                      }
                      alt={name || "User"}
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      <h2 className="text-2xl font-bold text-slate-900 dark:text-white md:text-3xl">{name || "Unnamed User"}</h2>

                      <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold capitalize ${statusStyles}`}>
                ...
                <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 md:p-6">
                  <div className="mb-5 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">Basic account and profile information.</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <InfoCard icon={<BadgeCheck className="h-5 w-5 text-slate-700 dark:text-slate-200" />} label="Full Name" value={name || "N/A"} />
                    ...
                    <InfoCard
                      icon={<Phone className="h-5 w-5 text-slate-700 dark:text-slate-200" />}
                      label="Phone"
                ...
                <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 md:p-6">
                  <div className="mb-5 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">Unique account reference details.</p>
                    </div>
                  </div>
                  ...
                  >
                    <p className="text-xs uppercase tracking-[0.2em] text-white/70">Profile Summary</p>
                    <h4 className="mt-2 text-lg font-semibold">{name || "Unnamed User"}</h4>
                    <p className="mt-1 text-sm text-white/75">
                      This account currently holds the <span className="font-semibold capitalize text-white">{role || "unknown"}</span> role and is
                      marked as <span className="font-semibold capitalize text-white">{displayStatus}</span>.
                    </p>
                  </motion.div>
                </div>
              </div>
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className={`mt-2 break-all text-base font-semibold text-slate-900 dark:text-white ${capitalize ? "capitalize" : ""}`}>{value}</p>
        </div>
      </div>
    </motion.div>
  );
...
const StatCard = ({ label, value }: { label: string; value: string }) => {
  return (
    ...
    >
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{label}</p>
      <p className="mt-2 text-base font-bold capitalize text-slate-900 dark:text-white">{value || "N/A"}</p>
    </motion.div>
  );
};
